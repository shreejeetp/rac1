...
/*
//todo
    1.Unpack: independent of utilization,try always unpacking the lowest utilized.DONE
    2.Merge: Try merging vm from all the PM's.
    3.Make separate function for crossover.Done
    4.Do heuristics after crossover and merge separately. DONE
    5.Rearrangement:Add priority wise removal of two containers from gene which can replace a bigger single container-to-be-allocated. DONE
    6.Apply a filter which removes any containers which is above max VM Size.
    7.Don't Delete parent. DONE
    8.Mutation using low utilized VM.Done
*/
        ...
        for(vmptr vm1=pm1->hvm;vm1!=NULL;vm1=vm1->next){
            ...
            for(cnptr cn1=vm1->hcn; cn1!=NULL; cn1=cn1->next){
                ...
                for(cnptr cn2=cn1->next; cn2!=NULL; cn2=cn2->next){
                    ...
                    cn_absent[absent_count]=cn2->ogi;
                    absent_count++;
...
int bf_vm_new(int cn_ogi){
    ...
    for(int i=0;i<VM_SIZE;i++){
        ...
        float cpu_diff=vm_cpu-cpu_req;
        if(best_cpu>cpu_diff){
            best_cpu=cpu_diff;
            vmi=i;
        }
